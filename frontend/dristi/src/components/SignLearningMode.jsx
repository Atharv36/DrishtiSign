...
export default function SignLearningMode({ close }) {
  ...
  const advanceLockRef = useRef(false); // Prevent the popup firing more than once per card
  ...
  // Untimed: the user can take as long as they need. Once they get it right,
  // show a success popup, then move on to the next sign after it closes.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Fallback: If python server crashes/drops a frame, unlock after 500ms so it doesn't freeze permanently
    let lastSendTime = 0;
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start h-[600px] relative">
          ...
          <div className="md:col-span-8 bg-gray-100 dark:bg-black rounded-2xl overflow-hidden border border-gray-200 dark:border-gray-800 shadow-inner relative h-full flex flex-col items-center justify-center">
            ...
            {!isActive ? (
                <div className="text-center p-8 flex flex-col items-center">
                    ...
                    <p className="text-gray-400 dark:text-gray-500 text-sm mt-2 max-w-xs">Start the camera to begin the lesson. Take your time — there's no clock here.</p>
                </div>
            ) : processedImage ? (
                <>
          ...
          <div className="md:col-span-4 flex flex-col h-full space-y-4">
            ...
            <div className={`flex-1 rounded-2xl p-6 flex flex-col items-center justify-center text-center transition-colors duration-500 border shadow-sm ${isCorrect ? 'bg-emerald-50 dark:bg-emerald-900/20 border-emerald-500' : 'bg-gray-50 dark:bg-gray-800/50 border-gray-200 dark:border-gray-800'}`}>
                ...
                <p className="text-xs text-gray-400 mt-4">{cardIndex + 1} / {flashcards.length}</p>
            </div>
