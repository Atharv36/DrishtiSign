...
export default function SignToText({ close }) {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      if (e.key === ' ' || e.code === 'Space') {
        ...
        e.preventDefault();
        socketRef.current?.emit('text_key', { action: 'backspace' });
      }
    };
  ...
  const clearSentence = () => {
    socketRef.current?.emit('text_key', { action: 'clear' });
  };
      ...
      if (isActive && videoRef.current && canvasRef.current) {
        if (!isProcessingRef.current || (timestamp - lastSendTime > 500)) {
            ...
            if (video.videoWidth > 0 && video.videoHeight > 0) {
                ...
                socketRef.current.emit('text_frame', base64Data);
            } else {
                isProcessingRef.current = false;
            }
        }
                ...
                <>
                    ...
                    <div className="absolute bottom-4 left-4 right-4 flex justify-between items-end">
                        <div className="bg-black/70 backdrop-blur-md p-3 rounded-xl border border-white/10 text-white min-w-[200px]">
                            <p className="text-xs uppercase tracking-wider text-gray-300 mb-1">Holding</p>
          ...
          <div className="md:col-span-4 flex flex-col h-full space-y-4">
            ...
            <div className="flex-1 rounded-2xl p-5 flex flex-col bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-800 shadow-sm">
                ...
                <p className="text-xs text-gray-400 mt-4 leading-relaxed">
                    Hold a letter steady to add it. Press <span className="font-semibold text-gray-500 dark:text-gray-300">Space</span> for a word break and <span className="font-semibold text-gray-500 dark:text-gray-300">Backspace</span> to remove the last letter.
                </p>
            </div>
